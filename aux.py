...
class SameLineLogger:
    ...
    def step(self, global_batch_idx, epoch_batch_idx, epoch_idx, loss, lr, grad):
        ...
        msg = f"[{passed_time}] {epoch_batch_idx} / {self.epoch_len}, epoch {epoch_idx + 1} / {self.epoch_num}, loss={loss}, avg {time_per_batch}/iter ({global_batch_idx} / {self.totalx}), {prediction} to finish, LR={lr:.2e}, grad={grad:.2e}        "
